...
df["account_percent_change"] = 0
df["contributions_percent_change"] = 0
df["grants_percent_change"] = 0
df["assets_percent_change"] = 0

# df.drop("0", axis=1, inplace = True)

for i, val in enumerate(df.iloc):
    if df.iloc[i, 0] == df.iloc[i, 8]:
        if df.iloc[i, 10] != 0:
            df.iloc[i, 14] = (df.iloc[i, 2] - df.iloc[i, 10])/df.iloc[i, 10]
        if df.iloc[i, 11] != 0:
            df.iloc[i, 15] = (df.iloc[i, 3] - df.iloc[i, 11])/df.iloc[i, 11]
        if df.iloc[i, 12] != 0:
            df.iloc[i, 16] = (df.iloc[i, 4] - df.iloc[i, 12])/df.iloc[i, 12]
        if df.iloc[i, 13] != 0:
            df.iloc[i, 17] = (df.iloc[i, 5] - df.iloc[i, 13])/df.iloc[i, 13]
